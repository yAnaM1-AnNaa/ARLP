...
def load_pano_detector(cfg):
    pano_type = cfg.get('pano_detector') # groundingdino; yoloworld
    pano_mode_path = cfg.get('pano_detector_path')
    print(f"Loading pano detector: {pano_type}, path: {pano_mode_path}")


def load_local_inferer(cfg):
    local_type = cfg.get('local_inferer') # steervit; latefilm; earlyfilm
    local_mode_path = cfg.get('local_inferer_path')
    print(f"Loading local inferer: {local_type}, path: {local_mode_path}")
...
def eval():
    ...
    # parser.add_argument("--local-backend", default=None, help="Local affordance backend: film/arlp or steer/steervit. Defaults to config.")
    ...
    load_local_inferer(cfg)
    load_pano_detector(cfg)
    ...
    ##### Generate eval text query #####
    # Since the finetune uses chain of thought prompt, so the text query could be really long.
    # Choice 1: use existing text query.
    # Choice 2: use text query generated by LLM though API calling.
    eval_set_info = []
    disambiguation_set = {
    ...
    dataset_pairs = {
        ...
        for obj in os.listdir(os.path.join(agd_gt_dir, action_name))

...
from src.get_vlm_response import get_end_to_end_matching
...
def find_best_camera_angle(h5_file_path, top_k=3):
    ...
    对于H5文件中的每个实例，找到与实例描述（通常是类别名）CLIP余弦相似度最高的相机视角。
    即：找到最能代表该物体的 top-k 个视角。
    无返回值，但会将 CLIP 相似度和 top-k 帧索引保存回 h5 文件中。
    """
    # 定义计算余弦相似度的内部函数
    def find_similarity(features_flat, query_feature):
        # 计算点积并除以范数（归一化），得到余弦相似度
    ...
    with h5py.File(h5_file_path, 'r') as h5_file:
        ...
        for instance_key in instance_keys:
            ...
            # get the rgb images
            # 获取该实例的所有 RGB 图像，(N, H, W, 3)其中N是视角的总数
            rgb_images = instance['rgb'][:]

            all_image_features = []
            # 遍历每一个图像，提取 CLIP 视觉特征
            ...
            clip_similarities = find_similarity(all_image_features, text_embeds)#计算得到该视角的相似性
            print(clip_similarities.shape)
            
            # Get top-k frame indices (ordered by similarity)其中top-k是指定的，例如3个最佳视角
...
def process_instance(
    ...
    # -------------------------------------------------------------------------
    # 2.5) Save fused point cloud as .ply（without color）
    # -------------------------------------------------------------------------
    # if hasattr(fusion, "points") and fusion.points is not None:
    #     points = fusion.points
    #     colors = getattr(fusion, "colors", None)

    #     if hasattr(points, "cpu"):
    #         points = points.cpu().numpy()
    #     if colors is not None and hasattr(colors, "cpu"):
    #         colors = colors.cpu().numpy()

    #     ply_path = os.path.join(
    #         os.path.dirname(query_original_path),
    #         f"{instance.name.strip('/')}_fusion.ply"
    #     )
    ...
    #     print(f"Saved fused point cloud to {ply_path}")
    ...
    # ---------------------------------------------------------------------
    # 3.5) Save clustered point cloud (.ply with cluster colors)
    # ---------------------------------------------------------------------
    if hasattr(fusion, "points") and fusion.points is not None:
        clustered_points = fusion.points
        if hasattr(clustered_points, "cpu"):
            clustered_points = clustered_points.cpu().numpy()

        cluster_rgb = np.zeros_like(clustered_points, dtype=np.float32)
        for label in unique_labels:
            mask = (cluster_results == label)
            rgb = np.array(mcolors.to_rgb(used_colors[label]), dtype=np.float32)
            cluster_rgb[mask] = rgb

        clustered_ply_path = os.path.join(
            os.path.dirname(query_original_path),
            f"{instance.name.strip('/')}_clustered.ply"
        )

        pcd_clustered = o3d.geometry.PointCloud()
        pcd_clustered.points = o3d.utility.Vector3dVector(clustered_points)
        pcd_clustered.colors = o3d.utility.Vector3dVector(cluster_rgb)
        o3d.io.write_point_cloud(clustered_ply_path, pcd_clustered)
        print(f"Saved clustered point cloud to {clustered_ply_path}")
...
def process_category(category_h5_path, dinov2, query_save_dir, embedding_type, text_embedding_func,use_existing_cluster=True, use_data_link_segs=False, top_k=3):
    ...
    with h5py.File(category_h5_path, 'r+') as h5_file:
        ...
        for instance_key in tqdm(instance_keys):
            ...
            # VLM matching per-instance (only when API key is present)
            if "OPENAI_API_KEY" in os.environ:
                region_matching = get_end_to_end_matching(
                    obj=category_name,
                    query_path=query_save_dir,
                    query_prefix=[f"{category_name}_{instance_key}"],
                    use_vlm='gpt'
                )
                ...
                print("Skipping VLM matching: OPENAI_API_KEY not set; only saving图像与点云。")
